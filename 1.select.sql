...
----------- order by 정렬기준,    asc(오름차순) , desc(내림차순)
----------- asc(오름차순) , desc(내림차순) 생략 금지
select 검색하고자하는컬럼명 from table명 or by 정렬기준컬럼명 asc? desec?


실행 순서 : from 절 -> select절 -> order by절
...
실행 순서 : from -> where -> select 순

***********************************************************

4. 조건절 & order by syntax
...
select distinct empno as 사번 from emp order by empno asc;


-- 10.emp table 에서 deptno 내림차순 정렬 적용해서 ename과 deptno 검색하기


select distinct deptno from emp order by deptno deasc;
...
잉여 table
임시 table로 가령 syntax 문법 오류 방지용으로 주로 사용
...
-- 데이터가 null인 경우엔 연산 자체가 무효화 모두가 다 null로 일괄 처리
-- 해결책 : null 은 연산 불가 따라서 null값을 0으로 치환해서 연산
...
select sal, comm, sal*12 as 연봉 from emp;
...
--15. comm이 null이 아닌 사원에 대한 검색(ename, comm)
	...
	select ename, comm from emp order by emp asc where comm is not null;

-- 15 문제에 comm을 오름차순으로 정렬해 주세요
...
--? 검색된 데이터의 sal 값이 내림차순desc으로 정렬order by 검색select
...
select ename as 사원이름, deptno, job from emp where deptno=10 or job='MANAGER';
...
-- Applicationd에서 sql 문장 사용시 권장형식
SELECT ename, deptno FROM emp WHERE deptno != 10;



--23. sal이 2000 이하(sal<=2000)이거나(or) 3000이상인(sal>=3000) 사원명(ename) 검색
select ename as 사원명, sal from emp where sal<=2000 or sal>=3000;
...
-- 81이라는 표현과 일치되는 방법? date의 결과에서 yy만 도출해서 비교	

-- 범위로 비교 하는 연산식 : between 여기서 and 저기까지
...
select ename, empno from emp where ename like 'M_';
